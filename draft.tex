...
Unfortunately, solving exact Survivable Set Connectivity is $\mathsf{NP}$-hard \cite{ssc}. Therefore, the focus of this paper is on an approximate solution to a relaxation of this problem given in \cite{ssc}. Instead of requiring at least $k$ edge-disjoint paths between each pair, we require that each pair have at least $\Omega(\frac{k}{\log n})$ edge-disjoint paths. With this relaxation, it is possible to find a subgraph with cost $O(polylog(n))$ times the optimum.

The paper is organized as follows. We first present a natural integer linear programming formulation of the problem. Then, we introduce the idea of a \emph{tree embedding}, which allows us to transform arbitrary graphs $G$ into trees that can be more easily handled by the algorithm. We also show a general rounding scheme, called RoundGKR, which our algorithm uses as a black box. Finally, we combine all these concepts together to present and analyze an approximation algorithm for SSC. 
%
...
We will use a particular type of tree embedding given in \cite{racke} which will allow us to map paths in the tree to paths in the graph.
...
\caption{R\"{a}cke's tree cut induced by an edge in the tree.}
\end{figure}
...
Figure~\ref{fig:racketree} gives an example of the flow correspondence between a graph and it's associated R\"{a}cke's tree. Each red vertex of the tree is mapped to another red vertex on the graph, and the edge between them corresponds to a path between the red vertices. This gives us a simple way to map flows from Racke trees to the graph and vice-versa. 

\begin{figure}
\label{fig:racketree}
\centering
\includegraphics[width=0.8\linewidth]{Rackeembeddinggraph.pdf}
...
The important property of the algorithm can be summarized in the following lemma, whose proof can be found in [CITE].
...
So the probability that a given edge of the tree is included in a particular sampling of a Racke tree is $O(\log^3 n) y_{e'}$. 

Using this, we can bound the probability that a particular edge $e$ of the original graph is included. For $e$ to be included, on one of the $\tau$ trials, the algorithm must chose some tree edge $e'$ such that $e' \in \text{map}^{-1}(e)$ (recall that this is the set of tree edges which, if selected, result in edge $e$ being included). So let $X_e$ be the indicator random variable which says that edge $e$ is included in $H$. Then we have the inequality
\begin{align}
 \textbf{E}[X_e] &\leq \tau \sum_{e' \in \text{map}^{-1}(e)} O(\log^3 n) y_{e'} 
\end{align}
The $\tau$ comes from a union bound over all trials. The $O(\log^3 n)y_{e'}$ comes from the first part of the proof, where we bounded the probability of including a specific $e'$. 

To simplify this inequality, we apply a series of definitions and lemmas stated earlier in Section~\ref{sec:rackestrees}. We can rewrite the quantity on the right-hand side as $O(\log^3 n) \tau E[load(e)]$ due to the definition of $load$. Using the definition of $rload$, this can be rewritten again as $O(\log^3 n) \tau E[rload(e) x_e']$. By  Lemma~\ref{lem:rload}, so we can bound this above by $O(\log^3 n) \tau \frac{4}{k} \alpha x_e$. Since $\alpha$ is bounded by $O( \log n)$, we get our final inequality of 
\begin{align}
