...
Figure~\ref{fig:racketree} gives an example of the flow correspondence between a graph and it's associated R\"{a}cke's tree. Each red vertex of the tree is mapped to another red vertex on the graph, and the edge between them corresponds to a path between the red vertices. This gives us a simple way to map flows from R\"{a}cke's trees to the graph and vice-versa. 
...
There are some crucial properties of R\"{a}cke's trees which we will treat as black boxes. We will define these as lemmas without proof, and we refer to [CITE] for the proofs.
...
\li Compute linear program SSC-LP and set to zero any $x_e < \dfrac{k}{n^2}$.
\li Let $x' \leftarrow x*\frac{4}{k}$. 
\li Prepare $\mathcal{D}$, the distribution of R'\"{a}cke's tree embeddings for $G$ with $x'$.
\li \For $\tau$ rounds \Do
\li Sample $(T, y, \text{map})$ from $\mathcal{D}$. 
\li \For $q = 0, ..., 2\log n$ \Do
