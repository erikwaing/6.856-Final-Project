...
\begin{itemize}
\item (Claim A) at least $\Omega(\frac{k}{\log n})$ edge disjoint paths between each $(S_i, T_i)$ pair
\item (Claim B) cost within a polylogarithmic factor of the optimum
\end{itemize}

We first prove Claim A, and then using Claim A, we prove Claim B.
...
\begin{align}
height(T) \sum_q \frac{4}{2^{-q}} 2^{q+1} y_{e'} \tau' &= height(T) \tau' y_{e'} \sum_q 2 = O(\log^3 n) y_{e'} 
\end{align}

So the probability that a given edge of the tree is included in a particular sampling of a Racke tree is $O(\log^3 n) y_{e'}$. 

For the some edge $e$ to be included, on one of the $\tau$ trials, there must be some $e' \in \mathrm{map}^{-1}(e)$. 

So now in order to include some edge $e$ of the graph $G$, we must include some edge $e' \in \text{map}^{-1}(e)$ (recall that this is the set of tree edges which, if selected, result in edge $e$ being included). So let $X_e$ be the indicator random variable which says that edge $e$ is included in $H$, and $Y_{e'}$ be the indicator random variable which says that edge $e'$ in the tree is included in some RoundGKR execution. Above, we argued that $E[Y_{e'}] \leq O(\log^3 n)y_{e'}$.
\begin{align}
