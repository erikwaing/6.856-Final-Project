...
&= O(\log^3 n) y_{e'} 
\end{align}
This is because $height(T) = O(\log n)$, $\tau' = O(\log n)$, and $q$ takes $O(\log n)$ possible values. So the probability that a given edge of the tree is included in a particular sampling of a Racke tree is $O(\log^3 n) y_{e'}$. 
